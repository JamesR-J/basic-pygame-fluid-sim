...
class Wall:
    ...
    def draw(self, surface):
        pygame.draw.line(surface, (50,120,100), (self.x1, self.y1), (self.x2, self.y2), width=5)
...
class Game():
    ...
    def render(self):    
        ...
        self.mouse_x, self.mouse_y = pygame.mouse.get_pos()
        speedtext = self.myfont.render(str(self.mouse_x) + " and " + str(self.mouse_y), 1, (0,0,0))
    ...
    def add_wall_coord(self):
        ...
        if len(self.wall_list) == 2 and self.mouse_x not in self.wall_list and self.mouse_y not in self.wall_list:
            ...
            self.walls.append(Wall(self.wall_list[0], self.wall_list[1],
                              self.wall_list[2], self.wall_list[3]))
            self.wall_list = []
